...
test("activityFor keeps song metadata and splits lyrics across the status and image caption", () => {
...
test("activityFor omits the image caption when there is no following lyric", () => {
  const result = activityFor(snapshot, [{ startMs: 0, text: "last line" }], 2, 10_000);
  assert.equal(result.activity.state, "last line");
  assert.equal(result.activity.assets.large_text, undefined);
});
